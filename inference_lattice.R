...
s.lattice.likelihood.vector <- function(s, m, N, fr, h=0.5){
  k <- dim(fr)[1]
  s <- matrix(s, nrow=k, ncol=k, byrow=TRUE)
...
## 2 or k*k+1 with the last element being m
...
lattice.s.hat <- function(fr, m, N, h=0.5){
  k <- dim(fr)[1]
  result <- optim(rep(0.05,k*k), s.lattice.likelihood.vector, m=m, N=N, fr=fr, h=h)$par
...
approx.lattice.s.hat.ij <- function(i,j,f,M, h=0.5){
  ...
  if(i<k){term2=term2-sum(hi*f[i+1,j,1:G])}
  if(i>1){term2=term2-sum(hi*f[i-1,j,1:G])}
  if(j<k){term2=term2-sum(hi*f[i,j+1,1:G])}
...
##   if(h!=0.5){stop("non-genic selection not implemented for constant s")}
  ...
  term1 <- matrix(0,k,k)                #1-m|k|
  term2 <- matrix(0,k,k)                #f_G - f_0 + sum(m|k|fij - sum(fi'j'))
  term3 <- matrix(0,k,k)                #f(1-f)
  ...
  return(matrix(sum(term2)/sum(term3)/2,k,k))
}
...
approx.lattice.s.hat.ij.em <- function(i,j,call,M, h=0.5){
  ...
  if(i<k){term2=term2-sum(exp.ht*f[i+1,j,(1:G)])}
  if(i>1){term2=term2-sum(exp.ht*f[i-1,j,(1:G)])}
  if(j<k){term2=term2-sum(exp.ht*f[i,j+1,(1:G)])}
...
approx.lattice.m.hat <- function(f,s, h=0.5){
  ...
  G <- dim(f)[3]
  mrm <- mig.rate.mat(k)
  if(!is.matrix(s)){s <- matrix(s,k,k)}

  term1 <- array(0, dim=c(k,k,G-1))     #f_{ij}^t-\mu_{ij}^t
  term2 <- array(0, dim=c(k,k,G-1))     #|k|\mu_{ij}^t-\sum \mu_{i'j'}
  term3 <- array(0, dim=c(k,k,G-1))     #f(1-f)

  fij <- f[,,1:(G-1)]
  hij <- fij+h*(1-2*fij)
  
  het <- f[,,1:(G-1)]*(1-f[,,1:(G-1)])
  term1 <- f[,,2:G]-f[,,1:(G-1)]-array(apply(het,3,'*',s),dim=c(k,k,G-1))*hij*2
  term2 <- array(apply(f[,,1:(G-1)],3,'*',mrm),dim=c(k,k,G-1))
  for(i in 1:k){
    for(j in 1:k){
      if(i<k){term2[i,j,]=term2[i,j,]-f[i+1,j,1:(G-1)]}
      if(i>1){term2[i,j,]=term2[i,j,]-f[i-1,j,1:(G-1)]}
      if(j<k){term2[i,j,]=term2[i,j,]-f[i,j+1,1:(G-1)]}
...
approx.lattice.m.hat.em <- function(call, h=0.5){
  ...
  term1 <- array(0,dim=c(k,k,G))
  term2 <- array(0,dim=c(k,k,G))
  ...
  for(i in 1:k){
    for(j in 1:k){
      ...
      for(t in 1:G){
        B <- states*mrm[i,j]
        if(i<k){B <- B-sum(states*fb[i+1,j,,t])}
        if(i>1){B <- B-sum(states*fb[i-1,j,,t])}
        if(j<k){B <- B-sum(states*fb[i,j+1,,t])}
...
iterate.approx.mle <- function(f, initial.s=0, h=0.5, max.iters=100, eps=0.001, s.free=TRUE){
  k <- dim(f)[1]
  if(!is.matrix(initial.s)){initial.s <- matrix(initial.s,k,k)}
...
##       f <- hmm.solved$viterbi
      ...
      s.hats <- array(0,dim=c(k,k,params$n.paths))
